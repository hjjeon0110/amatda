...
public class AdminController {
	...
	public ModelAndView QNASelectOne(@RequestParam int bNo, HttpSession session, Model model) {
	...
	public String statistics(Model model) {
		...
		// 운동종목별
		List<Statistics> Exercise = as.Exercise();
		...
		for (int i = 0; i < Exercise.size(); i++) {
			// System.out.println("for문 작동");
			// System.out.println(Exercise.get(i).getExercise());

			if (Exercise.get(i).getExercise().equals("맨몸운동")) {
				body += Exercise.get(i).getCount();
			} else if (Exercise.get(i).getExercise().equals("웨이트")) {
				weight += Exercise.get(i).getCount();
			} else if (Exercise.get(i).getExercise().equals("필라테스")) {
				pilates += Exercise.get(i).getCount();
			} else {
				yoga += Exercise.get(i).getCount();
			}

		}
		;

		// System.out.println("맨몸운동 : " + body + " 웨이트 : " + weight + " 필라테스 : " +
		// pilates + " 요가 : " + yoga);
		  ...
		  exercise.add(body);
		  exercise.add(weight);
		  exercise.add(pilates);
		  exercise.add(yoga);
		  model.addAttribute("exercise", exercise);
		  
		  
		List<Member> membershipCount = as.membershipCount();
